...
function parse_mat(file::String; kwargs...)::Dict
...
function mat_to_exapf(data_mat)
    ...
    for (i, branch) in enumerate(data_mat["branch"])
        ...
        branch_array[i, BR_B] = branch["br_b"]
        branch_array[i, RATE_A] = branch["rate_a"]
...
function export_matpower(io::IO, data::Dict{String, Any})
    ...
    if length(storage) > 0
        ...
        for (idx, strg) in sort(storage)
            if idx != strg["index"]
                @info("The index of the storage does not match the matpower assigned index. Any data that uses storage indexes for reference is corrupted.")
            end
            println(
                io,
                "\t",
    ...
    for (idx, branch) in sort(branches)
        ...
        println(
            ...
            "\t",
            (branch["b_to"] + branch["b_fr"]) != NaN ? (branch["b_to"] + branch["b_fr"]) :
            0,
            "\t",
    ...
    _export_extra_data(
        ...
        data,
        "load",
        Set(["index", "source_id", "load_bus", "status", "qd", "pd"]);
        postfix = "_data",
    )
    ...
    _export_extra_data(
        ...
        data,
        "shunt",
        Set(["index", "source_id", "shunt_bus", "status", "gs", "bs"]);
        postfix = "_data",
    )
